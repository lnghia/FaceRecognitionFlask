...
def predict_image_as_name(image):
    name = 'not known'
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
def delete_image_dir(id):
    try:
        shutil.rmtree(os.path.join(config.data_dir, id))
        return 1
    except OSError as e:
        print(f'Error: {config.data_dir} : {e.strerror}')
        return 0

def delete_feature_file():
    try:
        os.remove(os.path.join(config.feature_dir, 'weights.pkl'))
        return 1
    except OSError as e:
        print(f'Error: {config.feature_dir} : {e.strerror}')
        return 0